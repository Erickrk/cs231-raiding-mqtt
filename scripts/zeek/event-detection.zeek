...
global host_mqtt_qos2: table[string] of vector of time &default=vector();
...
event mqtt_publish(c: connection, is_orig: bool, msg_id: count, msg: MQTT::PublishMsg) &priority=5
{
    ...
    print "mqtt_publish event triggered";
    print fmt("QoS level: %d", msg$qos);
    if (pi$qos_level == 2) {
        local uid = c$uid;
        local now = network_time();
        host_mqtt_qos2[uid] += now;

        while ( |host_mqtt_qos2[uid]| > 0 && now - host_mqtt_qos2[uid][0] > 300secs ) {
            host_mqtt_qos2[uid] = host_mqtt_qos2[uid][1:];
        }

        if ( |host_mqtt_qos2[uid]| >= 20 ) {
            print fmt("Connection %s has published 20 messages with QoS 2 in less than 5 minutes", uid);
            host_mqtt_qos2[uid] = vector();
        }
    }
}
